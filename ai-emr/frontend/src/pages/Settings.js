...
export default function Settings() {
  ...
  const verifyMasterCode = async (master_code, new_password) => {
    try {
      const res = await authAPI.emergencyReset(master_code, new_password);
      return { success: true, ...res };
    } catch (e) { return { success: false, error: e.message }; }
  };

  const updateMasterCode = async (current_master_code, new_master_code) => {
    try {
      const res = await authAPI.updateMasterCode(current_master_code, new_master_code);
      return { success: true, ...res };
    } catch (e) { return { success: false, error: e.message }; }
  };

  const getAuditLog = async (limit = 100) => {
    return await authAPI.getAuditLog(limit);
  };
  ...
  const handleEmergency = async () => {
    ...
    if (res.success) {
      setEmgSucc("Admin password reset via master code.");
      setEmgCode(""); setEmgNew(""); setEmgCon("");
  ...
  const handleMasterCodeUpdate = async () => {
    ...
    if (res.success) {
      setMcSucc("Master code updated successfully.");
      setMcCurrent(""); setMcNew(""); setMcCon("");
      ...
      {tab === "passwords" && (
        <div>
          ...
          <div className="card" style={{ borderTop: "2px solid #DC2626" }}>
            <div className="card-header">
              <div style={{ fontWeight: 700, fontSize: 15 }}>Emergency Admin Reset</div>
            ...
            <div className="card-body">
              ...
              <div className="alert alert-amber">Use master backup code to reset Admin password if locked out.</div>
              <div className="form-group">
              ...
              <button className="btn btn-danger" onClick={handleEmergency}>Reset Admin Password</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div className="alert alert-amber">
              The master backup code is used for emergency admin password reset. Keep it secret. Min 8 characters.
            </div>
            <div className="form-group">
          ...
          <div style={{ maxHeight: 500, overflowY: "auto" }}>
            {auditLoading ? (
              ...
              <div key={entry.id} style={{ display: "grid", gridTemplateColumns: "160px 100px 1fr", gap: 12, padding: "10px 20px", borderBottom: "1px solid #F1F5F9", fontSize: 12 }}>
                ...
                <div><span className="badge badge-blue" style={{ fontSize: 10 }}>{entry.role || "—"}</span></div>
                <div>
