...
VALID_ROLES = ("admin", "doctor", "technician", "receptionist")
...
def emergency_reset(req: EmergencyResetRequest):
    ...
    db.execute("UPDATE credentials SET password=? WHERE role='doctor'", (hash_password(req.new_password),))
    log_action(db, "emergency_reset", role="doctor", detail="Doctor password reset via master code")
    db.commit()
    db.close()
